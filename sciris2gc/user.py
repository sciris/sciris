    ...
    def deleteByUID(self, theUID):
        ...
        if validUID is not None:
            ...
            if theObject is not None:
                # Remove entries from both user_dict and usernameHashes 
                # attributes.
...
def get_scirisdemo_user(name='_ScirisDemo'):    
    ...
    theUser = user_dict.getUserByUsername(name)
...
def user_login(userName, password):
    ...
    matchingUser = user_dict.getUserByUsername(userName)
...
def get_all_users():
    ...
    # Return success.
    return user_dict.getAdminFrontEndRepr()
...
def admin_delete_user(userName):
    ...
    user_dict.deleteByUsername(userName)
